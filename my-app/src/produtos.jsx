...
const API_URL = 'https://crudcrud.com/api/27300a56e81b4903a17fa21eb90a0039/produtos'
...
function ListaProdutos() {
    ...
    const handleToggleCarrinho = (id) => {
        if(carrinho.includes(id)) {
            setCarrinho(carrinho.filter(itemId => itemId !== id));
        } else {
        ...
        <Main>
            ...
            <Div2>
                {produtos.map((item)=>(
                    ...
                    adicionado={carrinho.includes(item._id)}
                    onAdicionar={()=> handleToggleCarrinho(item._id)}
                    />
                ))}
            </Div2>
            )}
